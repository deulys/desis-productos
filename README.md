...
- Servidor local: servidor embebido de PHP
...
Si se ejecutaron los datos iniciales mas de una vez y aparecen registros
duplicados, se puede recrear la base:

```bash
dropdb desis_productos
createdb desis_productos
psql -d desis_productos -f sql/crear_tablas.sql
psql -d desis_productos -f sql/cargar_datos_iniciales.sql
```
